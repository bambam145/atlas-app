...
export const ob = { step: 0, name: '', picks: [], times: {}, error: '', reto: -1 };
export const obMoment = (i) => ob.times[i] || HABIT_SUGGESTIONS[i][2] || 'libre';

// Si ya tienes hábitos, solo falta el nombre.
export const obSteps = () => (state.habits.length ? ['name'] : ['name', 'habits', 'moment', 'reto']);
...
export function renderOnboarding() {
  ...
  if (key === 'name') {
    ...
    if (ob.reto === -1 && ob.picks.length) ob.reto = ob.picks[0];
    body = `
      <h1 class="ob-title">¿Empiezas con un reto? <span>21 días cambian todo.</span></h1>
      <p class="auth-p">Elige un hábito y cúmplelo 21 días (no hace falta que sean seguidos). Al terminar ganas una medalla 🏅.</p>
      <div class="choice-opts">${ob.picks.map((i) => `
        <button class="choice-opt${ob.reto === i ? ' is-on' : ''}" data-action="ob-reto" data-v="${i}"><span class="pause-emoji">${HABIT_SUGGESTIONS[i][0]}</span><span>21 días: ${esc(HABIT_SUGGESTIONS[i][1])}</span>${ob.reto === i ? icon('check') : ''}</button>`).join('')}
        <button class="choice-opt${ob.reto === -2 ? ' is-on' : ''}" data-action="ob-reto" data-v="-2"><span class="pause-emoji">🙂</span><span>Ahora no, gracias</span>${ob.reto === -2 ? icon('check') : ''}</button>
      </div>
      <div class="ob-actions">
        <button class="text-btn" data-action="ob-back">${icon('left')} Atrás</button>
        <button class="pill" data-action="ob-finish">${ob.reto >= 0 ? 'Empezar mi reto' : 'Armar mi sistema'} ${icon('arrowRight')}</button>
      </div>`;
  } else {
    body = `
      ...
      <div class="ob-actions">
        <button class="text-btn" data-action="ob-back">${icon('left')} Atrás</button>
        <button class="pill" data-action="ob-next">Continuar ${icon('arrowRight')}</button>
      </div>`;
  }
